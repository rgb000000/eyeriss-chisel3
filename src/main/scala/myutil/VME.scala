// Virtual Memory Engineer  (copy from VTA VME)
package myutil
...
class VMEMaster(implicit p: Parameters) extends Bundle {
  val nRd = p(Shape)._1 + p(Shape)._1 + p(Shape)._2
  val nWr = 1
...
class VME(implicit p: Parameters) extends Module {
  ...
  val nReadClients = p(Shape)._1 + p(Shape)._1 + p(Shape)._2 - 1
...
class XilinxShellVME(implicit val p: Parameters) extends RawModule{
...
object GetVerilogVME extends App{
  implicit val p = new DefaultConfig
  chisel3.Driver.execute(Array("--target-dir", "test_run_dir/make_XilinxVME_test"), () => new XilinxShellVME)
}
